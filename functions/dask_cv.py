...
def dask_cv(estimator, X: da.Array, y: da.Array, cv_splits: int = 10, classes: list = None, hpo: bool = False,
    ...
    for train_split_idx, test_split_idx in splits:
        estimator = fit_estimator(estimator, X[train_split_idx], y[train_split_idx], classes)
        score = prediction_score(estimator, X[test_split_idx], y[test_split_idx], hpo, regression)
        scores.append(score)
        gc.collect()

    return dask.compute(*scores)
