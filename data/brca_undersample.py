...
brca_fpkm = pd.read_csv('brca_data/brca_fpkm.csv')
brca_subtypes = pd.read_csv('brca_data/brca_subtypes.csv')

print(brca_fpkm.shape)

n_top_var = 20000
sorted_var = brca_fpkm.var(axis=0).sort_values(ascending=False)
brca_fpkm_top_var = brca_fpkm[sorted_var.iloc[:n_top_var].index]

brca_fpkm_top_var.to_csv(f"brca_undersample/csv/n{brca_fpkm_top_var.shape[0]}_f{n_top_var}_brca_fpkm.csv",
                         index=False)
brca_subtypes.to_csv(f"brca_undersample/csv/n{brca_subtypes.shape[0]}_brca_subtypes.csv", index=False)
...
for nsub in num_subsamples:
    ...
    subsample_brca_fpkm.to_csv(f"brca_undersample/csv/n{nsub}_f{n_top_var}_brca_fpkm.csv", index=False)
    subsample_brca_subtypes.to_csv(f"brca_undersample/csv/n{nsub}_brca_subtypes.csv", index=False)
    ...
    dd.to_parquet(dd.from_pandas(subsample_brca_fpkm, npartitions=npts),
                  f"brca_undersample/parquet/n{nsub}_f{n_top_var}_brca_fpkm.parquet",
                  engine='pyarrow', compression='snappy')
    dd.to_parquet(dd.from_pandas(subsample_brca_subtypes, npartitions=1),
                  f"brca_undersample/parquet/n{nsub}_brca_subtypes.parquet",
                  engine='pyarrow', compression='snappy')
...
for nfeat in num_features:
    ...
    print(sorted_var.shape)
    print(feat_sel_brca_fpkm.shape)

    feat_sel_brca_fpkm.to_csv(f"brca_undersample/csv/n{brca_fpkm.shape[0]}_f{nfeat}_brca_fpkm.csv", index=False)
    dd.to_parquet(dd.from_pandas(feat_sel_brca_fpkm, npartitions=1),
                  f"brca_undersample/parquet/n{brca_fpkm.shape[0]}_f{nfeat}_brca_fpkm.parquet",
                  engine='pyarrow', compression='snappy')